...
layout(push_constant) uniform PushConsts {
	...
	layout (offset = 36) uint numSamples;
} consts;
...
vec2 hammersley2d(uint i, uint N) 	//生成低差异序列
{
...
vec3 importanceSample_GGX(vec2 Xi, float roughness, vec3 normal) 	//重要性采样
{
...
vec3 prefilterEnvMap(vec3 R, float roughness)
{
	...
	for(uint i = 0u; i < consts.numSamples; i++) {	
		...
		vec2 Xi = hammersley2d(i, consts.numSamples);		//每轮生成一个低差异序列样本
		vec3 H = importanceSample_GGX(Xi, roughness, N);	//根据该样本生成一个采样用的半程向量（再计算L就对应反射波瓣）
